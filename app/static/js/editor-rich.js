(() => {
  ...
  function joinPoint(doc3, pos, dir = -1) {
    ...
    for (let d2 = $pos.depth; ; d2--) {
      ...
      if (before && !before.isTextblock && joinable2(before, after) && $pos.node(d2).canReplace(index, index + 1))
        return pos;
      if (d2 == 0)
        break;
  ...
  var ViewTreeUpdater = class {
    ...
    updateNextNode(node, outerDeco, innerDeco, view, index, pos) {
      for (let i = this.index; i < this.top.children.length; i++) {
        ...
        if (next2 instanceof NodeViewDesc) {
          ...
          if (!locked && next2.update(node, outerDeco, innerDeco, view)) {
            this.destroyBetween(this.index, i);
            if (next2.dom != nextDOM)
              this.changed = true;
            this.index++;
            return true;
          } else if (!locked && (updated = this.recreateWrapper(next2, node, outerDeco, innerDeco, view, pos))) {
            this.destroyBetween(this.index, i);
    ...
    recreateWrapper(next2, node, outerDeco, innerDeco, view, pos) {
  ...
  var NodePos = class _NodePos {
    constructor(pos, editor, isBlock2 = false, node = null) {
...
.ProseMirror-focused .ProseMirror-gapcursor {
  ...
  var ResizableNodeView = class {
    ...
    constructor(options) {
      ...
      if ((_a2 = options.options) == null ? void 0 : _a2.min) {
        this.minSize = {
          ...this.minSize,
          ...options.options.min
        };
      ...
      if ((_e2 = options.options) == null ? void 0 : _e2.className) {
        this.classNames = {
    ...
    applyConstraints(width, height, preserveAspectRatio) {
      ...
      if (!preserveAspectRatio) {
        ...
        if ((_a2 = this.maxSize) == null ? void 0 : _a2.width) {
...
`;
      ...
      while (i < lines.length) {
        ...
        if (nextLine.trim() === "") {
          const nextNonEmptyIndex = lines.slice(i + 1).findIndex((l3) => l3.trim() !== "");
      ...
      props: {
        handleClick: (view, pos, event) => {
          ...
          if (options.openOnClick) {
            ...
            const href = (_a2 = link.href) != null ? _a2 : attrs.href;
...
${nextLine.slice(indentLevel + 2)}`;
  ...
  var Image = Node3.create({
    ...
    renderMarkdown: (node) => {
      var _a2, _b, _c, _d, _e2, _f;
      const src = (_b = (_a2 = node.attrs) == null ? void 0 : _a2.src) != null ? _b : "";
  ...
  var we = /^ {0,3}(#{1,6})(?=\s|$)(.*)(?:\n+|$)/;
  var F = /(?:[*+-]|\d{1,9}[.)])/;
  var ie3 = /^(?!bull |blockCode|fences|blockquote|heading|html|table)((?:.|\n(?!\s*?\n|bull |blockCode|fences|blockquote|heading|html|table))+?)\n {0,3}(=+|-+) *(?:\n+|$)/;
  var oe = d(ie3).replace(/bull/g, F).replace(/blockCode/g, /(?: {4}| {0,3}\t)/).replace(/fences/g, / {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g, / {0,3}>/).replace(/heading/g, / {0,3}#{1,6}/).replace(/html/g, / {0,3}<[^\n>]+>\n/).replace(/\|table/g, "").getRegex();
  var ye = d(ie3).replace(/bull/g, F).replace(/blockCode/g, /(?: {4}| {0,3}\t)/).replace(/fences/g, / {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g, / {0,3}>/).replace(/heading/g, / {0,3}#{1,6}/).replace(/html/g, / {0,3}<[^\n>]+>\n/).replace(/table/g, / {0,3}\|?(?:[:\- ]*\|)+[\:\- ]*\n/).getRegex();
...
Please report this to https://github.com/markedjs/marked.`, e) {
  ...
  var COLUMN_START_PATTERN = /^\s*\[\[columns:(2|3)\]\]\s*$/i;
  var COLUMN_SEPARATOR_PATTERN = /^\s*\[\[col\]\]\s*$/i;
  var COLUMN_END_PATTERN = /^\s*\[\[\/columns\]\]\s*$/i;
  var WORKSHEET_TOKEN_PATTERN = /\[\[worksheet:([A-Za-z0-9\-_]+)(?:\|([^\]]+))?\]\]/gi;
  var IMAGE_WITH_ATTR_PATTERN = /!\[([^\]]*)\]\(([^)]+)\)\{:\s*([^}]+)\s*\}/g;
  var IMAGE_PATTERN = /!\[([^\]]*)\]\(([^)]+)\)/g;
  var AUDIO_PATTERN = /<audio\s+controls\s+src="([^"]+)"\s*><\/audio>/gi;
  k.setOptions({
    gfm: true,
    breaks: false
  });
    ...
    addAttributes() {
      return {
        ...
        dataAssetPath: {
          ...
          renderHTML: (attributes) => attributes.dataAssetPath ? { "data-asset-path": attributes.dataAssetPath } : {}
        }
      };
    },
    parseHTML() {
  ...
  var WorksheetLink = index_default.extend({
    addAttributes() {
      return {
        ...this.parent?.(),
        class: {
        ...
        dataWorksheetSlug: {
          ...
          renderHTML: (attributes) => attributes.dataWorksheetSlug ? { "data-worksheet-slug": attributes.dataWorksheetSlug } : {}
        }
      };
    }
  });
  var RichImage = index_default3.extend({
    addAttributes() {
      return {
        ...this.parent?.(),
        class: {
  ...
  function convertMarkdownImages(chunk, form) {
    let prepared = chunk.replace(IMAGE_WITH_ATTR_PATTERN, (_match, alt, path, classes) => {
      const classValue = classes.split(/\s+/).filter((item) => item.startsWith(".")).map((item) => item.slice(1)).join(" ");
  ...
  function markdownToEditorHtml(markdownText, form) {
    ...
    while (index < lines.length) {
      ...
      const columnHtml = columns.slice(0, count).map((column) => {
        const content = column || "_Columna vac\xEDa_";
...
`;
  ...
  function initializeBookDocuments(root2 = document) {
    root2.querySelectorAll("[data-book-document]").forEach((container) => {
      ...
      const setActivePage = (nextPageNumber, anchorId = null) => {
        ...
        pages.forEach((page) => {
          const isActive2 = Number(page.dataset.pageNumber) === boundedPageNumber;
          page.classList.toggle("is-active", isActive2);
        });
        ...
        const next2 = container.querySelector("[data-page-next]");
        if (previous) previous.disabled = boundedPageNumber === 1;
        if (next2) next2.disabled = boundedPageNumber === pages.length;
    ...
    return {
      ...
      content: Array.from({ length: count }, (_value, index) => ({
        ...
        content: [
          ...
          {
            type: "paragraph",
            content: [{ type: "text", text: `Escribe aqu\xED el contenido de la columna ${index + 1}.` }]
          }
        ]
      }))
    };
  ...
  function updateSaveSummary(form) {
    ...
    if (checklist) {
      ...
      if (files.length) items.push(`Se a\xF1adir\xE1n ${pluralize(files.length, "recurso nuevo", "recursos nuevos")}.`);
  ...
  function initializeRichEditor(form) {
    ...
    const editor = new Editor({
      ...
      extensions: [
        ...
        index_default4.configure({
          placeholder: "Empieza a escribir el material aqu\xED. Usa la cinta superior para estructurarlo."
        }),
    ...
    form.querySelectorAll("[data-rich-command]").forEach((button) => {
      button.addEventListener("click", () => {
        const command2 = button.dataset.richCommand;
        if (command2 === "paragraph") editor.chain().focus().setParagraph().run();
        if (command2 === "heading") editor.chain().focus().toggleHeading({ level: 2 }).run();
        if (command2 === "bold") editor.chain().focus().toggleBold().run();
        if (command2 === "italic") editor.chain().focus().toggleItalic().run();
        if (command2 === "bulletList") editor.chain().focus().toggleBulletList().run();
        if (command2 === "blockquote") editor.chain().focus().toggleBlockquote().run();
        if (command2 === "columns-2") editor.chain().focus().insertContent(buildColumnsContent(2)).run();
        if (command2 === "columns-3") editor.chain().focus().insertContent(buildColumnsContent(3)).run();
        if (command2 === "pagebreak") editor.chain().focus().insertContent({ type: "pageBreak" }).run();
        if (command2 === "worksheet") setMode(form, "library");
        if (command2 === "image") {
